...
% if length(A_sums) > maxNumPoints
%     A_sums = sampleFromMatrix(A_sums,maxNumPoints);
% end

for k=1:4
    obj{k}=gmdistribution.fit(sampleFromMatrix(A_sums',maxNumPoints),k);
...
for segment = A_noise_indices
    ...
    %skip segment 1 and last because range overlaps extremes of sample. Could add code to handle these times.
        if segment ~= 1 || segment ~= length_total_sample
            ...
            xx = count:(count+length(sample_noise)-1);
            if xx(end) > maxNumNoise
                q = find(xx == maxNumNoise);
                xx = xx(1:q);
            end
            noise(xx) = sample_noise(1:length(xx));
            count = count + length(xx);
