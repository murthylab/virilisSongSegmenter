...
%         lomb_sim_results{i} = lomb_sim(data,ipiStatsLomb.culled_ipi);
        ...
        %to use culled_ipi
        load(path_file,'data','ipiStats');
        lomb_sim_results{i} = lomb_sim(data,ipiStats.culled_ipi);
...
for i=1:numel(SNR)
    ...
    f{i} = freq;
    alpha{i} = sign;
    freq_range_of_interest = sign(freq > .016 & freq < .022);
    if numel(freq_range_of_interest) > 1
        best_alpha(i) = min(freq_range_of_interest);
        best_P(i) = max(Pow(freq > .016 & freq < .022));        
    end
end
